...
def main():
    ...
    points_polydata = render_3D_points(location, magnitude, time_seconds, data)
...
def render_3D_points(location, magnitude, time_seconds, data):


    # if I define a dosctring for a class A, then if I run help(A) , then I get the docstring output
    #DOCSTRING  -- should be used for a class
    min_x, max_x, min_y, max_y, min_z, max_z = location.GetBounds()
    min_strength, max_strength = magnitude.GetRange()
    min_time, max_time = time_seconds.GetRange()  # in seconds
    ...
    plane.SetOrigin(min_x,min_y, min_z)
    plane.SetPoint1(min_x, max_y, min_z)
    plane.SetPoint2(max_x, min_y, min_z)
    ...
    magnitude.SetName("strength")
    time_seconds.SetName("time")
    points_polydata = vtk.vtkPolyData()  # -- consists of vertices and edges
    points_polydata.SetPoints(location) #  # polydata gets the coordinates and represents them as vertices and edges
    points_polydata.GetPointData().AddArray(magnitude) # add the magnitude dimension
    points_polydata.GetPointData().AddArray(time_seconds)  # add the time dimension
    ...
    threshold_filter.SetInput(points_polydata) # data to filter from mapper

    min_filtered_strength = 2.8
    max_filtered_strength = 5.8
    ...
    colorTransferFunction.AddRGBPoint(min_filtered_strength + 0.2, 255.0/255.0, 153.0/255.0, 51.0/255.0)
    colorTransferFunction.AddRGBPoint(min_filtered_strength + 1.0, 255.0/255.0, 0, 0)

    colorTransferFunction.AddRGBPoint(min_filtered_strength + 1.5, 100.0/255.0, 0, 100.0/255.0)
    colorTransferFunction.AddRGBPoint(max_filtered_strength-1.0, 100.0/255.0, 0, 200.0/255.0)
    colorTransferFunction.AddRGBPoint(max_filtered_strength, 90.0/255.0, 0, 210.0/255.0)
